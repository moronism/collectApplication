(function(root) {
	...
	**/
	...
	HowardBuriedPoint.pageType='';
	
	HowardBuriedPoint.recordingBrowseTime;
    ...
    HowardBuriedPoint.setParames = function() {
       ...
       if(typeof _YS != "undefined") {
       	  for(var i in _YS) {
				...
				case 'pageType':
				    HowardBuriedPoint.pageType=_YS[i][1];
				    break;
       	  		case 'Url':
          ...
          if(_YS.userConfig) {
		    ...
		    if(_YS.recordingBrowseTimeFun){
				HowardBuriedPoint.recordingBrowseTime=_YS.recordingBrowseTimeFun;
		    }
       } else {
	...
	**/
    HowardBuriedPoint.event = function() {
		...
		var onVisibilityChange = function(){
			if (!document[hiddenProperty]) {  
				...
				HowardBuriedPoint.recordingBrowseTime(1);
				that.parmas.clickElement = 'ExecutiveOperation(页面重新开始进行计时)';	   
                HowardHepler.send(HowardHepler.changeJSON2Query(that.getParames()), that.url);
				//计时function
				//再重置SecondTimer
				globalPoint=true;
				if("user_video_cost"==HowardBuriedPoint.pageType){
			        //判断是否属于视频类如果是视频类开启此计时器
					//在重置MinuteTimer
					HowardBuriedPoint.initMinuteTimer();
					that.parmas.clickElement = 'ExecutiveOperation(页面激活操作逻辑开始记录时间再重置MinuteTimer)';	
				}
				if("user_book_cost"==HowardBuriedPoint.pageType){
				...
				that.setParames();
				HowardBuriedPoint.recordingBrowseTime(0);
			    //TODO 页面非激活(停止记录时间)
				that.parmas.clickElement = 'ExecutiveOperation(页面停止计时)';	
                //停止计时function				
                HowardHepler.send(HowardHepler.changeJSON2Query(that.getParames()), that.url);
				if("user_video_cost"==HowardBuriedPoint.pageType){
					//判断是否属于视频类如果是视频类开启此计时器
					//停止MinuteTimer
				   globalTimer.clearTimeoutTimer(globalTimer.minuteTimer);
				   that.parmas.clickElement = 'ExecutiveOperation(页面非激活操作逻辑停止记录时间停止MinuteTimer)';
				}
				if("user_book_cost"==HowardBuriedPoint.pageType){
					//判断是否属于文章类如果是文章类开启此计时器
					//停止SecondTimer
		...
		HowardHepler.on(document.body,'touchstart', function(e) {
			...
			if(true==globalPoint){
				if("user_video_cost"==HowardBuriedPoint.pageType){
					...
					//在重置MinuteTimer
					HowardBuriedPoint.initMinuteTimer();
					that.parmas.clickElement = 'ExecutiveOperation(touchstart事件重置MinuteTimer定时器)';
				}
				if("user_book_cost"==HowardBuriedPoint.pageType){
					//判断是否属于文章类如果是文章类开启此计时器
					//停止SecondTimer
	...
	globalTimer.controlMinuteTimerFunc=function(){
		...
		if(check){
			globalPoint=true;
			//在重置MinuteTimer
			HowardBuriedPoint.parmas.clickElement = 'ExecutiveOperation(点击确定继续记时)';	
			HowardBuriedPoint.recordingBrowseTime(1);
			HowardBuriedPoint.initMinuteTimer();
		}else{
			HowardBuriedPoint.recordingBrowseTime(0);
			HowardBuriedPoint.parmas.clickElement = 'ExecutiveOperation(点击取消退出记时)';	
			history.back();
		}
		HowardHepler.send(HowardHepler.changeJSON2Query(HowardBuriedPoint.getParames()), HowardBuriedPoint.url);	
	}
	...
	HowardBuriedPoint.initSecondTimer=function(){
		...
		globalTimer.secondTimer=window.setTimeout(globalTimer.controlSecondTimerFunc, 30000);
	}
	
	HowardBuriedPoint.initMinuteTimer=function(){
		//超过15分钟弹出窗口询问用户是否在线
		globalTimer.minuteTimer=window.setTimeout(globalTimer.controlMinuteTimerFunc, 9000);
	}
